...
def test_click():
    with sync_playwright() as p:
        ...
        # Break the mask to see if that's what's swallowing it!
        page.evaluate("""
            () => {
               ...
               scene.mapZones.forEach(z => {
                   z.list.forEach(c => {
                       if (c.type === 'Image' && c.mask) {
                           c.clearMask();
                       }
                   });
               });
        ...
        print(f"Tapping EXACT bounds {pos['x']}, {pos['y']} without masks")
