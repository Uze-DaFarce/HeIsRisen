...
## 2026-03-10 - Extending Keyboard Navigation Parity to Endgame Modals
**Learning:** While the primary game container might be focusable, inner-game overlay states (like the `EggZamRoom` explanation modals or the "PLAY AGAIN" endgame state) frequently lack explicit keyboard bindings. When building an accessible click-to-dismiss modal, simply allowing standard pointers isn't enough. Ensuring `Space` and `Enter` keybinds are wired directly to the dismiss handler ensures users can navigate the heavy reading portions seamlessly without a mouse.
**Action:** When implementing a `pointerdown` dismiss listener on full-screen modals or prominent endgame buttons, always parallel it with `Space` and `Enter` keyboard events (either via `this.input.keyboard.on` in Phaser or global `window.addEventListener` for generic resets) and rigorously test the unbinding of these listeners to prevent multi-fires.
